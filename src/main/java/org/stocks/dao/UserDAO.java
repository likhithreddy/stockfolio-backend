        ...
        try (ResultSet rs = stmt.getResultSet()) {
          if (rs.next()) {
            dashboard.put("userInfo", Map.of(
                    ...
                    "available_funds", rs.getBigDecimal("available_funds")
            ));
          }
        }
      }
...
//            ));
            ...
            profitStmt.setInt(1, portfolioId);
            ResultSet profitRs = profitStmt.executeQuery();
            BigDecimal profit = BigDecimal.ZERO;
            if (profitRs.next()) {
              profit = profitRs.getBigDecimal(1);
            }
            profitRs.close();
            profitStmt.close();
  ...
  public void updateUserPreferences(int userId, String sector, String riskLevel, boolean notify)
          throws Exception {
    try (Connection conn = dataSource.getConnection()) {
